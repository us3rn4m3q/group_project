...
func main() {
	...
	fmt.Println("xcore.by parser is running on :9004")
...
func WebScraper(s string) []Item {
	...
	s = url.QueryEscape(s)
	scrapeURL := "https://x-core.by/catalog/?q=" + s
	...
	c := colly.NewCollector(colly.AllowedDomains("x-core.by/", "www.x-core.by/", "x-core.by", "www.x-core.by"))

	c.OnHTML("div.catalog_item", func(e *colly.HTMLElement) {

		secondpart := ""

		price := e.ChildText("span[class=price_value]")
		price, secondpart, _ = strings.Cut(price, "&nbsp;")
		price = price + secondpart
		...
		if itemUrl != "" {
			...
			if err == nil && !parsedUrl.IsAbs() {
				baseUrl, _ := url.Parse("https://x-core.by")
				itemUrl = baseUrl.ResolveReference(parsedUrl).String()
			}
		}

		imgUrls := e.ChildAttrs("img", "src")
		imgUrl := ""
		if len(imgUrls) > 0 {
			imgUrl = imgUrls[0]
		}
