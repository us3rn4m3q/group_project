...
func main() {
	...
	fmt.Println("256bit.by parser is running on :9001")
...
func WebScraper(s string) []Item {
	...
	s = url.QueryEscape(s)
	scrapeURL := "https://256bit.by/catalog/?q=" + s

	c := colly.NewCollector(colly.AllowedDomains("256bit.by", "www.256bit.by", "256bit.by/", "www.256bit.by/"))

	c.OnHTML("div.inner_wrap.TYPE_1", func(e *colly.HTMLElement) {

		imgUrls := e.ChildAttrs("img", "data-src")
		imgUrl := ""
		if len(imgUrls) > 0 {
			imgUrl = imgUrls[0]
		}
		...
		if imgUrl != "" {
			...
			if err == nil && !parsedUrl.IsAbs() {
				baseUrl, _ := url.Parse("https://256bit.by")
				imgUrl = baseUrl.ResolveReference(parsedUrl).String()
			}
		}

		price := e.ChildText("span[class=values_wrapper]")

		priceParts := strings.Fields(price)
		cleanPrice := ""
		for i, part := range priceParts {
			if strings.Contains(part, "р") {
				cleanPrice = strings.Join(priceParts[0:i+1], "")
				break
			}
		}

		cleanPrice, _ = strings.CutSuffix(cleanPrice, "уб.")
		if strings.Contains(cleanPrice, "р") {
			cleanPrice = strings.Replace(cleanPrice, "р", " р", 1)
		}

		if cleanPrice == "" {
			cleanPrice = "Нет в наличии"
		}
		...
		oneItem := Item{
			Name:    e.ChildText("div[class=item-title]"),
			ItemUrl: itemUrl,
			Price:   cleanPrice,
			ImgUrl:  imgUrl,
		}
