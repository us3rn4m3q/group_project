...
func main() {
	...
	fmt.Println("ram.by parser is running on :9003")
...
func WebScraper(s string) []Item {
	...
	s = url.QueryEscape(s)
	scrapeURL := "https://ram.by/catalogsearch?q=" + s
	...
	c := colly.NewCollector(colly.AllowedDomains("ram.by/", "www.ram.by/", "ram.by", "www.ram.by"))

	c.OnHTML("div.item", func(e *colly.HTMLElement) {

		price := e.ChildText("div[class=price]")
		...
		if itemUrl != "" {
			...
			if err == nil && !parsedUrl.IsAbs() {
				baseUrl, _ := url.Parse("https://ram.by")
		...
		if imgUrl != "" {
			...
			if err == nil && !parsedUrl.IsAbs() {
				baseUrl, _ := url.Parse("https://ram.by/catalogsearch?q=")
				imgUrl = baseUrl.ResolveReference(parsedUrl).String()
			}
		}

		oneItem := Item{
			Name:    e.ChildText("div[class=title]"),
			ItemUrl: itemUrl,
			Price:   price,
			ImgUrl:  imgUrl,
		}
